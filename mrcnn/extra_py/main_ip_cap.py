...
if __name__ == '__main__':
    ...
    while True:
        ...
        if a != -1 and b != -1:
            ...
            frame0 = cv2.imdecode(np.fromstring(
            ...
            for i in range(numParticles):
                particles[i] = particle.Particle(random.randint(
                    0, frame0.shape[0]-1), random.randint(0, frame0.shape[1]-1), 1/numParticles)
            ...
            if ctr % 10 == 0:
                ...
                for i in range(N):
                    if r['class_ids'][i] == 1 and i == idS:
                        ...
                        particles = pf.actionModel(
                            particles, numParticles, boxX, boxY, meanX, meanY, covX, covY)
                        particles = pf.sensorModel(
                            particles, numParticles, boxX, boxY)
                ...
                visualize.display_instances(frame, r['rois'], r['masks'], r['class_ids'],
                                            class_names, r['scores'], "Real Time Detection", size, ax, fig, onlyPeds=True)
