...
import sys
import numpy as np
from ..bounding_box_utils.bounding_box_utils import convert_coordinates, iou
import logging
...
def decode_detections(y_pred,
    ...
    if input_coords == 'centroids':
        ...
        y_pred_decoded_raw = convert_coordinates(
            y_pred_decoded_raw, start_index=-4, conversion='minmax2corners')
    ...
    if normalize_coords:
        # Convert xmin, xmax back to absolute coordinates
        y_pred_decoded_raw[:, :, [-4, -2]] *= img_width
        # Convert ymin, ymax back to absolute coordinates
        y_pred_decoded_raw[:, :, [-3, -1]] *= img_height

    # 3: Apply confidence thresholding and non-maximum suppression per class

    # The number of classes is the length of the last axis minus the four box coordinates
    ...
    for batch_item in y_pred_decoded_raw:
        ...
        for class_id in range(1, n_classes):
            ...
            threshold_met = single_class[single_class[:, 0]
                                         > confidence_thresh]
            # If any boxes made the threshold...
            if threshold_met.shape[0] > 0:
                # ...perform NMS on them.
                maxima = _greedy_nms(threshold_met, iou_threshold=iou_threshold,
                                     coords='corners', border_pixels=border_pixels)
                ...
                # ...and write the maxima to the other columns...
        ...
        # If there are any predictions left after confidence-thresholding...
        if pred:
            ...
            if top_k != 'all' and pred.shape[0] > top_k:
                ...
                top_k_indices = np.argpartition(
                    pred[:, 1], kth=pred.shape[0]-top_k, axis=0)[pred.shape[0]-top_k:]
                # ...and keep only those entries of `pred`...
                pred = pred[top_k_indices]
        else:
            # Even if empty, `pred` must become a Numpy array.
            pred = np.array(pred)
...
def decode_detections_fast(y_pred,
    ...
    y_pred_converted[:, :, 1] = np.amax(y_pred[:, :, :-12], axis=-1)
    ...
    if input_coords == 'centroids':
        ...
        y_pred_converted = convert_coordinates(
            y_pred_converted, start_index=-4, conversion='centroids2corners')
    elif input_coords == 'minmax':
        # delta(pred) / size(anchor) / variance * variance == delta(pred) / size(anchor) for all four coordinates, where 'size' refers to w or h, respectively
        y_pred_converted[:, :, 2:] *= y_pred[:, :, -4:]
        # delta_xmin(pred) / w(anchor) * w(anchor) == delta_xmin(pred), delta_xmax(pred) / w(anchor) * w(anchor) == delta_xmax(pred)
        y_pred_converted[:, :, [
            2, 3]] *= np.expand_dims(y_pred[:, :, -7] - y_pred[:, :, -8], axis=-1)
        # delta_ymin(pred) / h(anchor) * h(anchor) == delta_ymin(pred), delta_ymax(pred) / h(anchor) * h(anchor) == delta_ymax(pred)
        y_pred_converted[:, :, [
            4, 5]] *= np.expand_dims(y_pred[:, :, -5] - y_pred[:, :, -6], axis=-1)
        # delta(pred) + anchor == pred for all four coordinates
        y_pred_converted[:, :, 2:] += y_pred[:, :, -8:-4]
        y_pred_converted = convert_coordinates(
            y_pred_converted, start_index=-4, conversion='minmax2corners')
    elif input_coords == 'corners':
        # delta(pred) / size(anchor) / variance * variance == delta(pred) / size(anchor) for all four coordinates, where 'size' refers to w or h, respectively
        y_pred_converted[:, :, 2:] *= y_pred[:, :, -4:]
        # delta_xmin(pred) / w(anchor) * w(anchor) == delta_xmin(pred), delta_xmax(pred) / w(anchor) * w(anchor) == delta_xmax(pred)
        y_pred_converted[:, :, [
            2, 4]] *= np.expand_dims(y_pred[:, :, -6] - y_pred[:, :, -8], axis=-1)
        # delta_ymin(pred) / h(anchor) * h(anchor) == delta_ymin(pred), delta_ymax(pred) / h(anchor) * h(anchor) == delta_ymax(pred)
        y_pred_converted[:, :, [
            3, 5]] *= np.expand_dims(y_pred[:, :, -5] - y_pred[:, :, -7], axis=-1)
        # delta(pred) + anchor == pred for all four coordinates
        y_pred_converted[:, :, 2:] += y_pred[:, :, -8:-4]
    else:
        raise ValueError(
    ...
    if normalize_coords:
        ...
        # Convert ymin, ymax back to absolute coordinates
        y_pred_converted[:, :, [3, 5]] *= img_height
...
def decode_detections_debug(y_pred,
    ...
    if input_coords == 'centroids':
        if variance_encoded_in_target:
            ...
            y_pred_decoded_raw[:, :, [-4, -3]] = y_pred_decoded_raw[:, :,
                                                                    [-4, -3]] * y_pred[:, :, [-6, -5]] + y_pred[:, :, [-8, -7]]
            # Decode the predicted box width and heigt.
            y_pred_decoded_raw[:, :, [-2, -1]] = np.exp(
                y_pred_decoded_raw[:, :, [-2, -1]]) * y_pred[:, :, [-6, -5]]
        else:
            # Decode the predicted box center x and y coordinates.
            y_pred_decoded_raw[:, :, [-4, -3]] = y_pred_decoded_raw[:, :, [-4, -3]] * \
                y_pred[:, :, [-6, -5]] * y_pred[:, :,
                                                [-4, -3]] + y_pred[:, :, [-8, -7]]
            # Decode the predicted box width and heigt.
            y_pred_decoded_raw[:, :, [-2, -1]] = np.exp(
    ...
    # Expand the last axis by one.
    y_pred_decoded_raw2 = np.zeros(
        (y_pred_decoded_raw.shape[0], y_pred_decoded_raw.shape[1], y_pred_decoded_raw.shape[2] + 1))
    y_pred_decoded_raw2[:, :, 1:] = y_pred_decoded_raw
    # Put the box indices as the first element for each box via broadcasting.
    y_pred_decoded_raw2[:, :, 0] = np.arange(y_pred_decoded_raw.shape[1])
    ...
    for batch_item in y_pred_decoded_raw:
        ...
        for class_id in range(1, n_classes):
            ...
            # ...keep only those boxes with a confidence above the set threshold.
            threshold_met = single_class[single_class[:, 1]
                                         > confidence_thresh]
            # If any boxes made the threshold...
            if threshold_met.shape[0] > 0:
                # ...perform NMS on them.
                maxima = _greedy_nms_debug(
                    threshold_met, iou_threshold=iou_threshold, coords='corners', border_pixels=border_pixels)
                # Expand the last dimension by one element to have room for the class ID. This is now an arrray of shape `[n_boxes, 6]`
                maxima_output = np.zeros(
                    (maxima.shape[0], maxima.shape[1] + 1))
                # Write the box index to the first column...
                maxima_output[:, 0] = maxima[:, 0]
                # ...and write the class ID to the second column...
                maxima_output[:, 1] = class_id
                # ...and write the rest of the maxima data to the other columns...
                maxima_output[:, 2:] = maxima[:, 1:]
                # ...and append the maxima for this class to the list of maxima for this batch item.
                pred.append(maxima_output)
        # Once we're through with all classes, keep only the `top_k` maxima with the highest scores
        pred = np.concatenate(pred, axis=0)
        ...
        if pred.shape[0] > top_k:
            # ...get the indices of the `top_k` highest-score maxima...
            top_k_indices = np.argpartition(
                pred[:, 2], kth=pred.shape[0]-top_k, axis=0)[pred.shape[0]-top_k:]
            # ...and keep only those entries of `pred`...
            pred = pred[top_k_indices]
        # ...and now that we're done, append the array of final predictions for this batch item to the output list
        y_pred_decoded.append(pred)
...
def get_num_boxes_per_pred_layer(predictor_sizes, aspect_ratios, two_boxes_for_ar1):
    ...
    for i in range(len(predictor_sizes)):
        if two_boxes_for_ar1:
            num_boxes_per_pred_layer.append(
                predictor_sizes[i][0] * predictor_sizes[i][1] * (len(aspect_ratios[i]) + 1))
        else:
            num_boxes_per_pred_layer.append(
                predictor_sizes[i][0] * predictor_sizes[i][1] * len(aspect_ratios[i]))
