...
weights_path = 'weights/vgg16_2_classes.h5'

TEST = "011"

model.load_weights(weights_path, by_name=True) #TODO: confirm weights are changing
...
# optim = Adam(lr=0.003, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
optim = SGD(lr=0.001, momentum=0.9, decay=0.0, nesterov=False)

ssd_loss = SSDLoss(neg_pos_ratio=3, alpha=1.0)

model.compile(optimizer=optim, loss=ssd_loss.compute_loss, metrics=['accuracy'])
...
val_dataset.create_hdf5_dataset(file_path='validate_dataset.h5',
                                resize=False,
                                variable_image_size=True,
                                verbose=True)
...
model_checkpoint = ModelCheckpoint(filepath="./output/"+TEST+"/ssd300epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5",
                                   monitor='val_loss',
                                   verbose=1,
                                   save_best_only=True,
...
schedule = PolynomialDecay(maxEpochs=20, initAlpha=1e-3, power=2)  # TODO
...
initial_epoch = 0
final_epoch = 20
steps_per_epoch = 24  # max(1, (r)//batch_size)
...
model.save("./output/"+TEST+"/trained_model.h5")
# %% [markdown]
# ## 6. Make predictions
#
